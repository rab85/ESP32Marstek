...
bool ExternalRequestedAutomaticMode = false;
bool ExternalManualMode = false;
int ExternalRequestedPower = 0;
...
marstek_load GetCurrentBatteryControlInfo(int hour) {
  return GetBatteryControlInfo(hour);
}
...
void ControlBattery(bool wifiConnected) {
  ...
  } else if (ExternalRequestedAutomaticMode) {
    ControlBatteryAutomatic(currentBatteryPower);
  } else if (ExternalManualMode) {
    ControlBatteryManuel(ExternalRequestedPower, currentBatteryPower);
  } else if (allowExternalPower == false) {
